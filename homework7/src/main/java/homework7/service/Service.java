...
public class Service {
    ...
    public static List<Painting> findAllPaintings() {
    ...
    public static Artist[] findAllArtistsByPaintingsId(String id) {
        ...
        Artist[] result = new Artist[artistPaintings.size()];
        for (int i = 0; i < result.length; i++) {
            Artist found = artistDao.findOne(artistPaintings.get(i).getArtistId());
            result[i] = found;
        }
        return result;
    }
    ...
    public static Painting[] findAllPaintingsByArtistsId(String id) {
        ...
        Painting[] result = new Painting[artistPaintings.size()];
        for (int i = 0; i < result.length; i++) {
            Painting found = paintingDao.findOne(artistPaintings.get(i).getPaintingId());
            result[i] = found;
        }
        return result;
    }
